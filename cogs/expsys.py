from discord import Embed, Member, utils, option
...
class ExperienceSystem(commands.Cog):
    ...
    async def lastseen(self, ctx, member: Member = None):
        ...
        await ctx.respond(content=answer_text)
    ...
    @option('sort_by', description='Choose how to sort leaderboard', choices=['rep', 'exp'], default='exp')
    async def leaderboard(self, ctx, sort_by: str = 'exp'):
        ...
        match sort_by:
            ...
            case _:
                return ctx.respond('wrong sorting was given')
        ...
        paginator = MyPaginator(pages=embeds_list)
    ...
    async def rank_work(self, ctx, member):
        member = member or ctx.author
        mrow = db.session.query(db.m).filter_by(id=member.id).first()
        if not mrow.inlvl:
            return await ctx.respond(content="You decided to opt out of the exp system before")
        if member.bot:
            await ctx.respond(content='Sorry, our system does not count experience for bots.')
    ...
    async def avatar_bridge(self, ctx, member: Member = None):
        ...
        embed = await self.avatar_work(ctx, member)
        await ctx.respond(embed=embed)

    @commands.user_command(name="View User Avatar")
    async def avatar_usercmd(self, ctx, member: Member):
        embed = await self.avatar_work(ctx, member)
    ...
    async def rank_bridge(self, ctx,  *, member: Member = None):
        ...
        await ctx.respond(file=await self.rank_work(ctx, member))

    @commands.user_command(name="View User Server Rank")
    async def rank_usercmd(self, ctx, member: Member):
        await ctx.respond(file=await self.rank_work(ctx, member), ephemeral=True)
    ...
    @option('member', description='Member to give rep to')
    async def rep(self, ctx, member: Member):
        """Give +1 to `@member`'s reputation ;"""
        if member == ctx.author or member.bot:
            await ctx.respond(content='You can\'t give reputation to yourself or bots')
        else:
