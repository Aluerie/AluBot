...
from utils.dcordtools import ansi
...
async def lm_work(ctx):
    await ctx.defer()
    res = try_get_gamerstats(ctx.bot, start_at_match_id=0, matches_requested=1)
    embed = Embed(colour=Clr.prpl)
    embed.set_author(name='Irene\'s last match id')
    embed.description = f'`{res[0].match_id}`'
    return await ctx.respond(embed=embed)


async def wl_work(ctx):
    await ctx.defer()
...
async def mmr_work(ctx):
    ...
    await ctx.respond(file=plt_to_file(fig, filename='mmr.png'))
...
class GamerStats(commands.Cog):
    ...
    slhgr = SlashCommandGroup('irene', 'Commands to stalk Irene dota stats')

    @slhgr.command(name='lm', description="View Irene's last played Dota 2 match")
    async def lm_slh(self, ctx):
        await lm_work(ctx)

    @commands.command(name='lm', brief=Ems.slash, aliases=['lastmatch'])
    async def lm_ext(self, ctx):
        """Irene's last played Dota 2 match id"""
        await lm_work(ctx)

    @slhgr.command(name='wl', description="Irene's Win - Loss ratio in Dota 2 games for today")
    async def wl_slh(self, ctx):
        await wl_work(ctx)

    @commands.command(name='wl', brief=Ems.slash, aliases=['winrate'])
    async def wl_ext(self, ctx):
        """Irene's Win - Loss ratio in Dota 2 games for today"""
        await wl_work(ctx)

    @slhgr.command(name='dh', description="Irene's Dota 2 Match History (shows last 100 matches)")
    async def dh_slh(self, ctx):
        await dh_work(ctx)

    @commands.command(name='dh', brief=Ems.slash, aliases=['dotahistory'])
    async def dh_ext(self, ctx):
        """Irene's Dota 2 Match History (shows last 100 matches)"""
        await dh_work(ctx)

    @slhgr.command(name='mmr', description="Irene's Dota 2 MMR Plot")
    async def mmr_slh(self, ctx):
        await mmr_work(ctx)

    @commands.command(name='mmr', brief=Ems.slash)
    async def mmr_ext(self, ctx):
