...
def first_page(ctx):
    return Embed(
        color=Clr.prpl,
        title=f'{ctx.bot.user.display_name} | $help Menu',
        ...
        name=f'{Ems.FeelsDankMan} `Fun`', inline=False,
        value='Commands to have fun with ;'
    ).add_field(
        name=f'{Ems.DankFix} `Tools`', inline=False,
        value='Commands providing useful tools'
    ).add_field(
        name=f'{Ems.PepoG} `Info`', inline=False,
        value='Commands to get some useful info'
    ).add_field(
        name=f'{Ems.Smartge} `Stats`', inline=False,
        value='Commands to get some stats/infographics'
    ).add_field(
        name=f'{Ems.peepoPlsStepOnMe} `Lewd`', inline=False,
        value='NSFW tier commands'
    ).add_field(
        name='📝 `Reminders and ToDo list`', inline=False,
        value='Commands to organize yourself a little bit'
    ).add_field(
        name=f'{Ems.peepoHappyDank} `Birthday`', inline=False,
        value='Set your birthday and get congratulations from the bot'
    ).add_field(
        name=f'{Ems.bubuAyaya} `Profile`', inline=False,
        value='Commands about member profile'
    ).add_field(
        name=f'{Ems.TwoBButt} `Aluerie`', inline=False,
        value='Commands about Aluerie'
    ).add_field(
        name=f'{Ems.PepoDetective} `Other features`', inline=False,
...
def features_page(ctx):
    return Embed(
        ...
        description=f'{Ems.PepoDetective} There is a list of not listed on other pages features. '
...
class ViewHelp(View):
    ...
    @button(label="", style=ButtonStyle.primary, emoji=Ems.peepoPlsStepOnMe)
    ...
    embed_dict = {
        ...
        'Birthday':
            Embed(
                ...
                description=
                f'{Ems.peepoHappyDank} There is a special {rmntn(Rid.bday)} role in this server '
        ...
        'Profile':
            Embed(
                ...
                description=
                f'{Ems.bubuAyaya} There is a profile system in this server: levelling experience, '
                f'currency, reputation, custom profile and many other things. '
                f'You can find here related commands.'
            ).set_footer(text=footer_text).set_thumbnail(url=thumb_url),
        'Aluerie':
            Embed(
                color=Clr.prpl, title="Aluerie related commands",
                description=
                f'{Ems.TwoBButt} There are some commands to see Aluerie\'s info such as dota 2 match history.'
            ).set_footer(text=footer_text).set_thumbnail(url=thumb_url),
