...
class Moderation(commands.Cog):
    ...
    async def warn(self, ctx, member: Member = None, *, reason="No reason provided"):
        ...
        if member is None:
            return await ctx.send("You didn\'t mention a user to warn.")
        if member.id == Uid.irene:
            return await ctx.send("You can't do that to Irene {}".format(Ems.bubuGun))
        if member.bot:
            return await ctx.send("Don't bully bots, please")
    ...
    async def ban(self, ctx, member: Member = None, *, reason="No reason provided"):
        ...
        if member is None:
            return await ctx.send("You didn\'t mention a user to ban.")
        if member.id == Uid.irene:
            return await ctx.send(f"You can't do that to Irene {Ems.bubuGun}")
        if member.bot:
            return await ctx.send("Don't bully bots, please")
    ...
    async def mute(self, ctx, member: Member = None, *, arg_string="No reason provided"):
        ...
        if member is None:
            return await ctx.send("You didn\'t mention a user to mute.")
        if member.id == Uid.irene:
            return await ctx.send(f"You can't do that to Irene {Ems.bubuGun}")
        if member.bot:
            return await ctx.send("Don't bully bots, please")
    ...
    async def unmute(self, ctx, member: Member = None):
        ...
        if member.id == Uid.irene:
            return await ctx.send(f"You can't do that to Irene {Ems.bubuGun}")
        if member.bot:
            return await ctx.send("You couldn't mute bots in the first place")
        ...
        await ctx.send(content=answer, embed=embed)
    ...
    async def check_mutes(self):
        for row in db.session.query(db.u):
            if row.dtime.replace(tzinfo=timezone.utc) < datetime.now(timezone.utc) + timedelta(minutes=30):
                ...
                self.bot.loop.create_task(
                    self.fire_the_unmute(row.id, row.name, row.userid, row.channelid, row.dtime))
    ...
    async def on_member_update(self, before: Member, after: Member):
        if before.timed_out != after.timed_out:
            embed = Embed(colour=Clr.red)
            embed.set_author(
                name=f'{after.display_name} is timed out',
