...
from utils.const import Colour, Emote, Guild, Role

if TYPE_CHECKING:
    from utils import AluBot


class Moderation(AluCog, emote=Emote.peepoPolice):
    ...
    async def warn(self, ctx: AluContext, member: discord.Member, *, reason: str = "No reason"):
        ...
        if member.id == self.bot.owner_id:
            raise commands.BadArgument(f"You can't do that to Aluerie {Emote.bubuGun}")
        if member.bot:
            raise commands.BadArgument("Don't bully bots, please")

        e = discord.Embed(title="Manual warning by a mod", colour=Colour.prpl(), description=reason)
    ...
    async def mute_work(ctx, member, dt: datetime.datetime, duration: datetime.timedelta, reason):
        try:
            ...
            e = discord.Embed(color=Colour.error(), description=f'You can not mute that member')
            e.set_author(name='MissingPermissions')
            return await ctx.reply(embed=e, ephemeral=True)

        e = discord.Embed(color=Colour.prpl(), title="Mute member")
    ...
    async def unmute(self, ctx: AluContext, member: discord.Member, *, reason: str = 'No reason'):
        ...
        e = discord.Embed(color=Colour.prpl(), title="Unmute member")
        e.description = f"{member.mention}, you are unmuted now, congrats {Emote.bubuSip}"
        e.add_field(name='Reason', value=reason)
        e.set_author(name=member.display_name, icon_url=member.display_avatar.url)
        content = member.mention if ctx.interaction else ''
        await ctx.reply(content=content, embed=e)
    ...
    async def on_member_update(self, before: discord.Member, after: discord.Member):
        if after.guild.id != Guild.community:
            return
