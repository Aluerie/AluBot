...
from utils import AluCog, AluContext, const, times
...
class ModerationCog(AluCog, emote=const.Emote.peepoPolice):
    ...
    async def mute_work(ctx, member, dt: datetime.datetime, duration: datetime.timedelta, reason):
        try:
            ...
            e = discord.Embed(color=const.Colour.error(), description=f'You can not mute that member')
            e.set_author(name='MissingPermissions')
            return await ctx.reply(embed=e, ephemeral=True)

        e = discord.Embed(color=const.Colour.prpl(), title="Mute member")
    ...
    @app_commands.default_permissions(manage_messages=True)
    @app_commands.command(name='mute', description="Mute+timeout member from chatting")
    @app_commands.describe(member='Member to mute+timeout', duration='Duration of the mute', reason='Reason')
    async def slh_mute(
        self, ntr: discord.Interaction, member: discord.Member, duration: str, *, reason: str = "No reason"
    ):
    ...
    async def unmute(self, ctx: AluContext, member: discord.Member, *, reason: str = 'No reason'):
        ...
        e = discord.Embed(color=const.Colour.prpl(), title="Unmute member")
        e.description = f"{member.mention}, you are unmuted now, congrats {const.Emote.bubuSip}"
        e.add_field(name='Reason', value=reason)
        e.set_author(name=member.display_name, icon_url=member.display_avatar.url)
        content = member.mention if ctx.interaction else ''
        await ctx.reply(content=content, embed=e)
