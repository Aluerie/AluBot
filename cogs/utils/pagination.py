...
class Paginator(discord.ui.View):
    ...
    async def on_error(self, ntr: discord.Interaction, error: Exception, item: discord.ui.Item) -> None:
        if ntr.response.is_done():
            await ntr.followup.send(f'{error} occurred, sorry', ephemeral=True)
        else:
            await ntr.response.send_message(f'{error} occurred, sorry', ephemeral=True)
