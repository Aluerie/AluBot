...
class MyTwitchClient(Twitch):
    ...
    async def last_vod_link(
        ...
        try:
            log.debug('last_vod_link')
            async for i in self.get_videos(user_id=str(user_id)):
                log.debug(i)
            log.debug('xd')
    ...
    async def get_live_lol_player_ids(self, pool: Pool) -> List[int]:
        ...
        query = f"""SELECT twitch_id, id
                ...
                """
        log.debug('starting to gather live lol streams')
        twitch_id_to_fav_id_dict = {r.twitch_id: r.id for r in await pool.fetch(query)}
        log.debug(f'{twitch_id_to_fav_id_dict}')
        ...
        log.debug(f'finished gathering lol live streams {live_twitch_ids}')
...
async def main():
    ...
    vod_link = await tc.last_vod_link(tw_id)
    print(vod_link)
