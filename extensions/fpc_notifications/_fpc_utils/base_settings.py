...
class FPCSettingsBase(FPCCog):
    ...
    * and probably more to come.
    ...
    async def database_list(self, ctx_ntr: AluGuildContext | discord.Interaction[AluBot]) -> None:
        ...
        columns = ", ".join(
            ["a.id", "p.lower_name", "display_name", "twitch_id", "a.lower_name"] + self.extra_account_info_columns
        )
        query = f"""SELECT {columns}
                    FROM {self.game}_players p
                    JOIN {self.game}_accounts a
                    ON p.lower_name = a.lower_name
        ...
        for row in rows:
            if row.lower_name not in player_dict:
                followed = (
                    " {0} {0} {0}".format(const.Emote.DankLove) if row.lower_name in favourite_player_list else ""
                )
                player_dict[row.lower_name] = {
            ...
            player_dict[row.lower_name]["info"].append(self.player_acc_string(**kwargs))
    ...
    async def get_player_dict(self, *, name_flag: str, twitch_flag: bool) -> dict:
        lower_name = name_flag.lower()
        if twitch_flag:
            twitch_id, display_name, profile_image = await self.bot.twitch.fpc_data_by_login(lower_name)
        else:
    ...
    async def check_if_already_in_database(self, account_dict: dict):
        query = f""" SELECT display_name, lower_name
        ...
        if user is not None:
            ...
            raise commands.BadArgument(
                ...
                f"Did you mean to use `/{self.game} player add name1: {user.lower_name}` to add the player into your fav list?"
            )
    ...
    async def database_add(
        ...
        query = f"""INSERT INTO {self.game}_players
                        (lower_name, display_name, twitch_id)
                            VALUES ($1, $2, $3)
                        ON CONFLICT (lower_name) DO NOTHING
                """
        await ctx.client.pool.execute(
            query, player_dict["lower_name"], player_dict["display_name"], player_dict["twitch_id"]
        )

        dollars = [f"${i}" for i in range(1, len(self.extra_account_info_columns) + 3)]  # [$1, $2, ... ]
        query = f"""INSERT INTO {self.game}_accounts
                    (lower_name, id, {', '.join(self.extra_account_info_columns)})
                    VALUES {'('}{', '.join(dollars)}{')'}
                """
        await ctx.client.pool.execute(query, player_dict["lower_name"], *account_dict.values())
    ...
    async def database_remove(
        ...
        account_id: Optional[Union[str, int]],  # steam_id for dota, summoner_id for lol
    ) -> None:
        """Base function for removing accounts from the database"""
        ctx = await self.get_ctx(ctx_ntr)
        await ctx.typing()
        if lower_name is None and account_id is None:
            raise commands.BadArgument("You need to provide at least one of flags: `name`, `steam`")

        if account_id:
            if lower_name:  # check for both lower_name and account_id
                query = f"""SELECT a.id 
                            FROM {self.game}_players p
                            JOIN {self.game}_accounts a
                            ON p.lower_name = a.lower_name
                            WHERE a.id=$1 AND p.lower_name=$2
                        """
                val = await ctx.client.pool.fetchval(query, account_id, lower_name)
                if val is None:
                    raise commands.BadArgument(
            ...
            query = f"""WITH del_child AS (
                            ...
                            WHERE  id = $1
                            RETURNING lower_name, id
                            )
                        DELETE FROM {self.game}_players p
                        USING  del_child x
                        WHERE  p.lower_name = x.lower_name
                        AND    NOT EXISTS (
                            SELECT 1
                            FROM   {self.game}_accounts c
                            WHERE  c.lower_name = x.lower_name
                            AND    c.id <> x.id
                            )
            ...
            # query for lower_name only
            query = f"""DELETE FROM {self.game}_players
                        WHERE lower_name=$1
                        RETURNING display_name
                    """
            ans_name = await ctx.client.pool.fetchval(query, lower_name)
            if ans_name is None:
    ...
    async def player_add_remove(
        ...
        query = f"SELECT lower_name FROM dota_favourite_players WHERE guild_id=$1"
        fav_names = [r for r, in await ctx.client.pool.fetch(query, ctx.guild.id)]
        query = f"""SELECT lower_name, display_name 
                    FROM {self.game}_players
                    WHERE lower_name=ANY($1)
                """
        ...
        # ids, which are lower_name for this
        success_ids = [row.lower_name for row in success_and_already_rows if row.lower_name not in fav_names]
        already_ids = [row.lower_name for row in success_and_already_rows if row.lower_name in fav_names]

        # display names
        success_names = [row.display_name for row in success_and_already_rows if row.lower_name not in fav_names]
        already_names = [row.display_name for row in success_and_already_rows if row.lower_name in fav_names]

        failed_names = [
            name for name in player_names if name.lower() not in [row.lower_name for row in success_and_already_rows]
        ]
